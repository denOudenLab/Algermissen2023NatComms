...
% Plots for Supplementary Figure S13.
% Same approach as used for Figure 4 and Supplementary Figure S03.
%
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13A');
...
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13A.csv'));
csvwrite(fullFileName, data.TF2plot);

% ----------------------------------------------------------------------- %
%% Figure S13B: Line plot theta power:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13B');
saveas(gcf, [figName '.png']);
...
% Save source data file:
% See job.condNames
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13B_Reward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 1, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13B_NoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 2, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13B_NoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 3, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13B_Punishment.csv'));
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/FigS13B_topo');
saveas(gcf, [figName '.png']);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13B_topo.csv'));
...
% ----------------------------------------------------------------------- %
%% Figure S13C: Line plot beta power:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13C');
saveas(gcf, [figName '.png']);
...
% Save source data file:
% See job.condNames
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13C_Reward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 1, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13C_NoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 2, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13C_NoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 3, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13C_Punishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 4, 1:until));
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/FigS13C_topo');
saveas(gcf, [figName '.png']);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13C_topo.csv'));
timeIdx         = dsearchn(data.topo2plot.time', sigTime'); timeIdx = timeIdx(1):timeIdx(end);
freqIdx         = dsearchn(data.topo2plot.freq', job.freq'); freqIdx = freqIdx(1):freqIdx(end);
saveMat         = squeeze(mean(mean(data.topo2plot.powspctrm(:, freqIdx, timeIdx), 3), 2));
saveMat         = horzcat(data.topo2plot.label, num2cell(saveMat));
...
% ----------------------------------------------------------------------- %
% ----------------------------------------------------------------------- %
%% Figure S13D: EEG correlates PE_BIAS:

% Within taft_postprocess_load_job, add the following setting:
...
% Exclude subjects:
job.invalidSubs       = [11 12 23 30]; % subjects excluded for EEG --> FOR UPDATESTD AND UPDATEDIF
...
% Save figure:
fig = gcf;
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13D');
saveas(gcf, [figName '.png'])
...
% Save source data file:
if strcmp(job.regNames(iROI), 'Updatebias')
    fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13D.csv');
    csvwrite(fullFileName, squeeze(real(tg)));
end
fprintf('Done :-)\n')

% ----------------------------------------------------------------------- %
%% Figure S13D: Topoplot:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13D_topo.png');
saveas(gcf, figName);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS13D_topo.csv'));
...
% ----------------------------------------------------------------------- %
%% Figure S13E & S13F: EEG correlates PE_STD and PE_DIF:
...
for iROI = [1 2] % iROI = 9; % UpdateStd and UpdateDif: Fig. 4
    ...
    if strcmp(job.regNames(iROI), 'Updatestd')
        figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13E');
        saveas(gcf, [figName '.png']);
        fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13E.csv');
        csvwrite(fullFileName, squeeze(real(tg)));
    end
    % Save source data file:
    if strcmp(job.regNames(iROI), 'Updatedif')
        figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13F');
        saveas(gcf, [figName '.png']);
        fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS13F.csv');
        csvwrite(fullFileName, squeeze(real(tg)));
    end
