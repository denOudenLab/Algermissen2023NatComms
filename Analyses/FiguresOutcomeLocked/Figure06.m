...
% Plots for Figure 6 in manuscript.
...
% Save:
figName = fullfile(dirs.root, sprintf('/Log/OutcomeLockedPaperPlots/Fig6ALeft'));
saveas(gcf, [figName '.png'])
pause(1)
close gcf

% Save source data file:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6ALeft.csv'));
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/Fig6ARight');
saveas(gcf, [figName '.png']);
...
% Save source data file:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6ARight.csv'));
csvwrite(fullFileName, data.TF2plot);
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/Fig6B');
saveas(gcf, [figName '.png']);
...
% Save source data file:
% See job.condNames
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_GoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 1, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_GoNoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 2, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_GoNoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 3, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_GoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 4, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_NoGoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 5, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_NoGoNoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 6, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_NoGoNoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 7, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6B_NoGoPunishment.csv'));
...
saveas(gcf, fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig6CLeft.png'));
close gcf
...
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6CLeft_cope_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
data            = niftiread(fullfile(dirs.data, sprintf('zstat1_cope%d_%s.nii', job.iCope, job.sign)));
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6CLeft_zstat_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
...
EEGfMRIPav_sliceDisplay_cope(dirs, job);
saveas(gcf, fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig6CRight.png'));
close gcf
...
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6CRight_cope_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
data            = niftiread(fullfile(dirs.data, sprintf('zstat1_cope%d_%s.nii', job.iCope, job.sign)));
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6CRight_zstat_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
...
EEGfMRIPav_sliceDisplay_cope(dirs, job);
saveas(gcf, fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig6DLeft.png'));
close gcf
...
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6DLeft_cope_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
data            = niftiread(fullfile(dirs.data, sprintf('zstat1_cope%d_%s.nii', job.iCope, job.sign)));
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6DLeft_zstat_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);

job.iCope = 1; job.iView = 'coronal'; job.iSlice = -30; job.zLim = [0 5]; job.cLim = 500; job.sign = 'pos';
EEGfMRIPav_sliceDisplay_cope(dirs, job);
saveas(gcf, fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig6DRight.png'));
close gcf
...
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6DRight_cope_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
data            = niftiread(fullfile(dirs.data, sprintf('zstat1_cope%d_%s.nii', job.iCope, job.sign)));
sliceIdx        = floor(size(data, 2)/2) + 9 + job.iSlice/2; % convert into MNI coordinates
saveMat         = squeeze(data(:, sliceIdx, :)); % imagesc(imrotate(saveMat, 90)); pbaspect([1 1 1]);
fullFileName    = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig6DRight_zstat_%s%d.csv', job.iView, job.iSlice));
csvwrite(fullFileName, saveMat);
