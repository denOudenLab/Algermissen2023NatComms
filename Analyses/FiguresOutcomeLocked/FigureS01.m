...
% Plots for Supplementary Figure S01.
% The same approach is used for Figure 2 and Supplementary Figures S06, S07. 
%
...
% Execute for figures S01A, S01B, S01C.
...
% --> Move on to 'Selecting subjects' and then Figures S01A, ...

% ----------------------------------------------------------------------- %
%% Alternative B: Load simulations: 	

% Execute for figures S01E, S01F, S01G.
...
% ======================================================================= %
% ======================================================================= %
%% Figures S01A & S01E: LEARNING CURVES:
...
if strcmp(datType, 'data')
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01A');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01E');
end
...
if strcmp(datType, 'data')
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01A_G2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 1, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01A_G2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 2, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01A_NG2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 3, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01A_NG2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 4, :)));
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01E_G2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 1, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01E_G2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 2, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01E_NG2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 3, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01E_NG2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 4, :)));
end
% ======================================================================= %
% ======================================================================= %
% ======================================================================= %
%% Figures S01B & S01F: p(Go) BAR PLOTS
...
% Determine figure name:
if strcmp(datType, 'data')
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01B');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01F');
end
saveas(gcf, [figName '.png']);
...
% Save source data files:
if strcmp(datType, 'data')
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01B_pGo.csv'));
    csvwrite(fullFileName, pGoCondMean);
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01B_pCorrect.csv'));
    csvwrite(fullFileName, pCorrectCondMean);
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01F_pGo.csv'));
    csvwrite(fullFileName, pGoCondMean);
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01F_pCorrect.csv'));
    csvwrite(fullFileName, pCorrectCondMean);
end
...
% ======================================================================= %
% ======================================================================= %
%% Figure S01C: Win-stay/ Lose-stay:
...
% Determine figure name:
if strcmp(datType, 'data')
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01C');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01G');
end
saveas(gcf, [figName '.png']);
...
% Save source data files:
if strcmp(datType, 'data')
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01C.csv'));
    csvwrite(fullFileName, data(validSubs, condOrder));
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01G.csv'));
    csvwrite(fullFileName, data(validSubs, condOrder));
end
...
% ======================================================================= %
% ======================================================================= %
%% Figure S01D: LOG MODEL EVIDENCE

% Connect data points of the same subject or not:
addLines    = false;

% Relevant settings:
modVec      = 1:5; modNames = modVec;
...
% ------------------------------------------------------ %
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01D');
saveas(gcf, [figName '.png']);
...
% Save source data files:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01D.csv'));
csvwrite(fullFileName, lME(validSubs, :));

% ======================================================================= %
% ======================================================================= %
% ======================================================================= %
%% Figure S01H: MODEL FREQUENCY AND EXCEEDANCE PROBABILITY:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS01H');
saveas(gcf, [figName '.png']);
...
% Save source data files:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01H_modFreq.csv'));
csvwrite(fullFileName, modFreq);
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS01H_PXP.csv'));
