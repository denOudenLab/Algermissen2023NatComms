...
% The same approach is used for Supplementary Figures S03 and S13.
%
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4A');
...
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4A.csv'));
csvwrite(fullFileName, data.TF2plot);

% ----------------------------------------------------------------------- %
%% Figure 4B: Line plot theta power:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4B');
saveas(gcf, [figName '.png']);
...
% Save source data file:
% See job.condNames
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4B_Reward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 1, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4B_NoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 2, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4B_NoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 3, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4B_Punishment.csv'));
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/Fig4B_topo');
saveas(gcf, [figName '.png']);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4B_topo.csv'));
...
% ----------------------------------------------------------------------- %
%% Figure 4C: Line plot beta power:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4C');
saveas(gcf, [figName '.png']);
...
% Save source data file:
% See job.condNames
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4C_Reward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 1, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4C_NoReward.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 2, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4C_NoPunishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 3, 1:until));
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4C_Punishment.csv'));
csvwrite(fullFileName, data.SubCondTime(job.validSubs, 4, 1:until));
...
% Save:
figName = fullfile(dirs.root, '/Log/OutcomeLockedPaperPlots/Fig4C_topo');
saveas(gcf, [figName '.png']);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4C_topo.csv'));
timeIdx         = dsearchn(data.topo2plot.time', sigTime'); timeIdx = timeIdx(1):timeIdx(end);
freqIdx         = dsearchn(data.topo2plot.freq', job.freq'); freqIdx = freqIdx(1):freqIdx(end);
saveMat         = squeeze(mean(mean(data.topo2plot.powspctrm(:, freqIdx, timeIdx), 3), 2));
saveMat         = horzcat(data.topo2plot.label, num2cell(saveMat));
...
% ----------------------------------------------------------------------- %
% ----------------------------------------------------------------------- %
%% Figure 4D: EEG correlates PE_BIAS:
...
if strcmp(job.regNames(iROI), 'Updatebias') && ~isfield(job, 'suffix'); pCrit = .20; end
...
% Save figure:
fig = gcf;
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4D');
saveas(gcf, [figName '.png'])
...
% Save source data file:
if strcmp(job.regNames(iROI), 'Updatebias')
    fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4D.csv');
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4D_topo.png');
saveas(gcf, figName);
...
% Save source data file:
fullFileName 	= fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/Fig4D_topo.csv'));
...
% ----------------------------------------------------------------------- %
%% Figure 4E & 4F: EEG correlates PE_STD and PE_DIF:
...
for iROI = [1 2]
    ...
    if strcmp(job.regNames(iROI), 'Updatestd')
        figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4E');
        saveas(gcf, [figName '.png']);
        fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4E.csv');
        csvwrite(fullFileName, squeeze(real(tg)));
    end
    % Save source data file:
    if strcmp(job.regNames(iROI), 'Updatedif')
        figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4F');
        saveas(gcf, [figName '.png']);
        fullFileName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/Fig4F.csv');
        csvwrite(fullFileName, squeeze(real(tg)));
    end
