...
% Plots for Figure S06A-H in manuscript.
% The same approach is used for Figure 2 and Supplementary Figures 1, 7. 
% Note that no data is plotted, only simulations.
% Select model 5 for S06A-C and model 6 for S06E-G.
%
...
% Set model:
iMod        = 5; % For figures S06A-C
% iMod        = 6; % For figures S06E-G

% Execute for figures S06A-C and S06E-G.
% Select model 5 for S06A-C and model 6 for S06E-G.
...
% ======================================================================= %
% ======================================================================= %
%% Figures S06A & S06E: LEARNING CURVES:
...
if iMod == 5
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06A');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06E');
end
...
if iMod == 5
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06A_G2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 1, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06A_G2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 2, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06A_NG2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 3, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06A_NG2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 4, :)));
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06E_G2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 1, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06E_G2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 2, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06E_NG2W.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 3, :)));
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06E_NG2A.csv'));
    csvwrite(fullFileName, squeeze(pGoCond(validSubs, 4, :)));
end
% ======================================================================= %
% ======================================================================= %
% ======================================================================= %
%% Figures S06B & S06F: p(Go) BAR PLOTS
...
if strcmp(datType, 'data')
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06B');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06F');
end
saveas(gcf, [figName '.png']);
...
% Save source data files:
if iMod == 5
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06B_pGo.csv'));
    csvwrite(fullFileName, pGoCondMean);
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06B_pCorrect.csv'));
    csvwrite(fullFileName, pCorrectCondMean);
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06F_pGo.csv'));
    csvwrite(fullFileName, pGoCondMean);
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06F_pCorrect.csv'));
    csvwrite(fullFileName, pCorrectCondMean);
end
...
% ======================================================================= %
% ======================================================================= %
%% Figure S06C: Win-stay/ Lose-stay:
...
% Determine figure name:
if strcmp(datType, 'data')
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06C');
else
    figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06G');
end
saveas(gcf, [figName '.png']);
...
% Save source data files:
if iMod == 5
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06C.csv'));
    csvwrite(fullFileName, data(validSubs, condOrder));
else
    fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06G.csv'));
    csvwrite(fullFileName, data(validSubs, condOrder));
end
...
% ======================================================================= %
% ======================================================================= %
%% Figure S06D: LOG MODEL EVIDENCE

% Connect data points of the same subject or not:
addLines    = false;

% Relevant settings:
modVec      = 1:6; % plus action priming model
modNames    = modVec;
...
% ------------------------------------------------------ %
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06D');
saveas(gcf, [figName '.png']);
...
% Save source data files:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06D.csv'));
csvwrite(fullFileName, lME(validSubs, :));

% ======================================================================= %
% ======================================================================= %
% ======================================================================= %
%% Figure S06H: MODEL FREQUENCY AND EXCEEDANCE PROBABILITY:
...
% Save:
figName = fullfile(dirs.root, 'Log/OutcomeLockedPaperPlots/FigS06H');
saveas(gcf, [figName '.png']);
...
% Save source data files:
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06H_modFreq.csv'));
csvwrite(fullFileName, modFreq);
fullFileName = fullfile(dirs.root, sprintf('Log/OutcomeLockedPaperPlots/FigS06H_PXP.csv'));
