...
GLMID=2 # don't use registration from pre-processing, but new registration computed at 1st-level GLM
...
for (( subject=1; subject<=36 ; subject++ )); do
	...
	subDir=${rootDir}/Log/fMRI/sub-${subjectID}/GLM${GLMID}/GLM${GLMID}_sub${subjectID}.feat

	# Invert transform from structural to standard space (takes ~2 min.)
	$fslDir/invwarp -w ${subDir}/reg/highres2standard_warp -o ${subDir}/reg/standard2highres_warp -r ${subDir}/reg/highres
