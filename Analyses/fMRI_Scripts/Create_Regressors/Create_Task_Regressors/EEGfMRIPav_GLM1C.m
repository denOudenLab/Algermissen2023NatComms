...
%
% Use group-level HBI parameters from M9 (with outcome reinterpretation
% parameter eta and perseveration parameter phi).
%
...
% tmp         = load(fullfile(dirs.cbm, '/HBI_Results/hbi_mod_19.mat'));
% modHBI      = tmp.cbm.output.group_mean{:}; % group-level

% 7 means:
modelParameters = [2.0408   -2.6905    0.1049    0.2059    1.5249   -0.0674    1.1418];
...
for iSub = 1:nSubjects % length(dirs.subs) % specify for which subjects to create regressors
    ...
    for iTrial = 1:nTrial
        ...
        % Reinterpet outcome:
        if r == 0
            r_eff = cueDetected(stim) * PavValues(stim) * eta;
        else
            r_eff = r;
        end
        ...
        if isnan(r) == 1 % no updating if outcome is NaN, i.e. subject got feedback that uninstructed key was pressed
            ...
            PEwithbias(iTrial)          = rho * r_eff - avwithbias(:, c, stim); % compute prediction error as reward minus expected value to be updated
