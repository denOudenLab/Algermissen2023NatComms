...
% taft_postprocess_time_topoplot(job, dirs, Tvalues, iROI, startTime, endTime, steps, zlim, isSave)
% 
...
nCol            = 5;
nRow            = ceil(length(startTimeVec) / nCol);
...
sgtitle(sprintf('Topoplot for ROI %s, time window %.02f - %.02f sec.', ...
    char(job.regNames(iROI)), startTimeVec(1), endTimeVec(end)), 'fontsize', fontSize);
...
if isSave
    saveas(gcf, fullfile(dirs.topoplot, sprintf('Topoplot_time_%s_%s_%dsec_%s-%sms.png', ...
