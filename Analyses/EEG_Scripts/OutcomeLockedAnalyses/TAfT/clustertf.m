...
%         tgr_thresh = tgr > thresh; % positive only
%         tgr_thresh = tgr < -1*thresh; % negative only
...
%         imlabel_pos = bwlabel(tgr > thresh); % labels for connected components (i.e. clusters) above thres
%         imlabel_neg = bwlabel(tgr < -1*thresh); % labels for connected components (i.e. clusters) above thres
%         imlabel = imlabel_pos + max(imlabel_pos(:)).*(imlabel_neg>0) + imlabel_neg; % positive + intercept for all negative + negative
