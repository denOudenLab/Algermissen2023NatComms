...
% nTrial      = size(X, 1): % number of trials
...
if isfield(job, 'behavReg')
    ...
    for iBehav = 1:nBehav
        ...
        xBehav(:, iBehav)   = tmp(job.goodTrlIdxfMRI, :); % select good trials after EEG trial rejection
    end
    ...
    if isempty(X) % if no fMRI ROIs specified
        X                               = xBehav;
