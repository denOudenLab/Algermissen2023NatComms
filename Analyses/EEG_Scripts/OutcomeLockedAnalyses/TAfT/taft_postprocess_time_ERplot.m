...
if any(Tvec < -1*tCrit & pvec < pCrit)
    ...
    patch([timeVec(sigTimeIdx) fliplr(timeVec(sigTimeIdx))], ...
        [repmat(-2, 1, sum(sigTimeIdx)) fliplr(Tvec(sigTimeIdx)')], 'b'); hold on
end
...
plot(chanBetas{iSub}.time, repelem(2, nTime), 'color', [0.5 0.5 0.5], ...
    'linestyle', '--', 'linewidth', lineWidth - 1); hold on
plot(chanBetas{iSub}.time, repelem(-2, nTime), 'color', [0.5 0.5 0.5], ...
...
sgtitle(sprintf('ER-plot for regressor %s, GLM-type %s, channels %s', ...
...
if isSave
    saveas(gcf, fullfile(dirs.TFplot, sprintf('ERplot_%s_%s_%dsec_%s.png', ...
        char(job.regNames(iROI)), job.HRFtype, job.trialdur, strjoin(selChans, ''))));
end
