...
% .description  = string, contains important information about job to be saved in output file name, includes EEGdomain, regNames.
% .EEGdomain 	= string, either 'TF' (data in TF domain) or 'time' (data in time domain).
...
% .toi          = vector of two numerics, time range of interest. 
...
if ~strcmp(EEGdomain, 'TF') && ~strcmp(EEGdomain, 'time') 
    error('Invalid EEGdomain specified'); 
end
...
% Timing (a bit longer than 1 sec., late effects seem to be more sustained):
% job.toi                     = [0 1.3]; 
job.toi                     = [-0.25 1.3]; 

% ERPs subtracted (induced):
% job.suffix                  = '_outERPcor_Go_all';

% Directory from which to load EEG data:
if strcmp(job.EEGdomain, 'time')
    job.EEGfile             = fullfile(dirs.EEG, ...
        sprintf('finalPP/MRIpav_%03d_finalPP.mat', job.subID)); % where EEG data are
    job.EEGname             = 'scd';
elseif strcmp(job.EEGdomain, 'TF')
    job.EEGdir              = sprintf('TF_subjectlevel_%s%02d', job.TFtype, job.nFreqs);
    if isfield(job, 'suffix'); job.EEGdir = [job.EEGdir job.suffix]; end
    job.EEGfile             = fullfile(dirs.EEG, job.EEGdir, ...
        sprintf('MRIpav_%03d_outcomelocked.mat', job.subID)); % where MEEG data are
    job.EEGname             = 'freq';
else
...
if ~isempty(behav2use) % if specified: add to regNames 
    warning('Behavioral regressors %s specified\n', strjoin(behav2use, ' '));
...
if ~strcmp(selTrials, 'all') % if specified: add to regNames
    ...
    job.selTrialsFile       = fullfile(dirs.TAfT, ...
        sprintf('TAfT_selTrials/selTrials_%s_Sub%03d.mat', job.selTrials, job.subID));
    job.regNames            = [job.regNames job.selTrials];
end

% ----------------------------------------------------------------------- %
%% Name of file with rejected trials:

job.rejTrialsfile          = fullfile(dirs.EEG, ...
    sprintf('rejTrials/MRIpav_%03d_rejTrials.mat', iSub));
...
if isempty(job.regNames{1}); job.regNames(1) = []; end % delete first entry if empty 

job.description             = sprintf('TAfT_%s_%s_%s_%dsec_%s',...
    job.EEGdomain, strjoin(job.regNames, '_'), ...
    job.HRFtype, job.trialdur, job.regType);

% Add type of TF decomposition and # frequencies:
if strcmp(job.EEGdomain, 'TF')
    job.description         = sprintf('%s_freq%02d', ...
        job.description, job.nFreqs); % add number of frequencies
end

% Add if special suffix:
if isfield(job, 'suffix')
    job.description         = sprintf('%s%s', ...
        job.description, job.suffix); 
end
...
for iROI = 1:length(ROIs2use) % loop through ROIs
   ...
   for iBlock = 1:nBlocks % loop through ROIs
        ...
        fMRIsubdir                                  = fullfile(dirs.fMRI, ...
            sprintf('sub-%03d/FEAT_Block%d.feat/AROMA', iSub, iBlock));

        % BOLD time series extracted from ROI:
        job.ROIs(iROI).ROIdef(iBlock).rawfMRIfile   = {fullfile(fMRIsubdir, ...
            sprintf('%s.txt', job.ROIs(iROI).ROIname))}; % where to load volumes from

        % Trial onsets (text file):
        job.ROIs(iROI).ROIdef(iBlock).onsets        = {fullfile(fMRIsubdir, ...
            'outcomeOnsets.txt')}; % trial onsets

        % Potential nuisance regressors:
        if ~strcmp(job.ROIs(iROI).ROIname, 'mp_Fellner')
            job.ROIs(iROI).ROIdef(iBlock).nuisance  = {fullfile(fMRIsubdir, ...
                'NuisanceRegressors.txt')}; % where to load volumes from
        end

        % Output directory:
        job.ROIs(iROI).ROIdef(iBlock).betafMRIfile  = {fullfile(dirs.TAfT, ...
            sprintf('betas_ROI_%s_sub%03d_block%d.txt', job.ROIs(iROI).ROIname, iSub, iBlock))};
...
fprintf('Create job in %s domain, %s-wise GLM for fitting HRF, trial duration %d sec., %s regression, \nRegressors: \n%s\n', ...
    job.EEGdomain, job.HRFtype, job.trialdur, job.regType, strjoin(job.regNames, '\n'));
