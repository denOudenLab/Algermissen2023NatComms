...
for iSub = 1:nSub
    ...
    contourf(chanBetas{iSub}.time, chanBetas{iSub}.freq, ...
    squeeze(chanBetas{iSub}.powspctrm), 30, 'linestyle', 'none');
    % colorbar
...
sgtitle(sprintf('TF-plot per subject for ROI %s, GLM-type %s, channels %s', ...
...
if isSave 
    fileName = sprintf('TFplot_%s_%s_%dsec_%s.png', ...
