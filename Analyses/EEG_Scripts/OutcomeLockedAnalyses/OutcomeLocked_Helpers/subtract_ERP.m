...
if strcmp(mode, 'Manual')
    ...
    if iscell(ERPdata.time)
        meanERP = nan(length(ERPdata.label), ...
            length(dsearchn(ERPdata.time{1}', ERPstart):dsearchn(ERPdata.time{1}', ERPdur)), ...
            length(ERPselTrials));
    else
        meanERP = nan(length(ERPdata.label), ...
            length(dsearchn(ERPdata.time', ERPstart):dsearchn(ERPdata.time', ERPdur)), ...
            length(ERPselTrials));
    end
