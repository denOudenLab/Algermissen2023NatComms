...
% figure('name', sprintf('%s TF power Outcome valence x  action', job.chanName), 'units', 'normalized', 'outerposition',[0 0 1 1]); hold on % fullscreen
...
cfg.colorbar    = 'no'; % if 'no', then add colorbar yourself later
cfg.figure      = gcf;
...
if isfield(job, 'freq')
    ...
    title(sprintf('%s vs. %s:\n %s (%d-%d Hz) power from %.3f to %.3f sec', ...
        job.twoLineLabels{1}, job.twoLineLabels{end}, job.bandName, ...
        job.freq(1), job.freq(end), ...
        job.sigTime(1), job.sigTime(end)), 'fontsize', fontSize);
 
else
    ft_topoplotER(cfg, data.topo2plot);
    title(sprintf('%s vs. %s:\n voltage from %.3f to %.3f sec', ...
        job.twoLineLabels{1}, job.twoLineLabels{end}, job.sigTime(1), job.sigTime(end)), 'fontsize', fontSize);
end
...
colorbar % such that also fontsize gets set appropriately
