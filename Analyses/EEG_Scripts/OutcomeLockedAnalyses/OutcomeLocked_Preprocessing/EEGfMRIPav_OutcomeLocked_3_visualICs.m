function EEGfMRIPav_OutcomeLocked_3_visualICs(rootDir, subVec)

% EEGfMRIPav_OutcomeLocked_3_visualICs(rootDir, subVec)
% 
...
for iSub = selIndices % iSub = 1;
    ...
    for iComp = 1:nComp
        ...
        set(gca, ...
            'clim', [-.5 * mean(abs(get(gca, 'clim'))), .5 * mean(abs(get(gca, 'clim')))], ...
        ...
        for iTrial = 1:nTrial
            plot(comp.time{1}, ...
                squeeze(compActivation(iComp, :, iTrial)), ...
                'Color', cmap(iTrial, :));
        end
        set(gca, 'xlim', comp.time{1}([1 end])); xlabel('Time (s)'); 
