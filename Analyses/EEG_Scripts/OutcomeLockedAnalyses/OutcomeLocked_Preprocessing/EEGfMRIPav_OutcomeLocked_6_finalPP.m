function EEGfMRIPav_OutcomeLocked_6_finalPP(rootDir, subVec)

% EEGfMRIPav_OutcomeLocked_6_finalPP(rootDir, subVec)
% 
...
fprintf('Number of rejected trials: M = %.03f, SD = %.03f, range %d - %d \n', ...
    mean(nRejTrial), std(nRejTrial), min(nRejTrial), max(nRejTrial));
% % Number of rejected trials: M = 117.722, SD = 117.509, range 11 - 499

% Reject invalid subjects:
invalidSubs = [11 12 23 30];
nRejTrial(invalidSubs)'
nRejTrial_keep = nRejTrial(setdiff(1:nSub, invalidSubs));
fprintf('Number of rejected trials: M = %.03f, SD = %.03f, range %d - %d \n', ...
    mean(nRejTrial_keep), std(nRejTrial_keep), min(nRejTrial_keep), max(nRejTrial_keep));
% Number of rejected trials: M = 81.875, SD = 54.025, range 11 - 194 
