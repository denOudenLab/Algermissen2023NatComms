function EEGfMRIPav_OutcomeLocked_1_preICA(rootDir, subVec)

% EEGfMRIPav_OutcomeLocked_1_preICA(rootDir, subVec)
% 
...
% Check how many channels rejected:
nChan = nan(size(channel2repair, 1), 1); % initialize
for iSub = 1:size(channel2repair, 1) % iSub = 1;
    nChan(iSub) = length(channel2repair{iSub, 2});
end

fprintf('Channels to repair: M = %.03f, SD = %.03f, range %d - %d \n', ...
    mean(nChan), std(nChan), min(nChan), max(nChan));
% Channels to repair: M = 4.889, SD = 3.012, range 1 - 13 

% ----------------------------------------------------------------------- %
%% Detect behavioral data files:

resultsfile     = dir(fullfile(dirs.behavior, '*_results.mat'));
...
for iSub = selIndices % iSub = 1;
    ...
    % Retrieve codes for each outcome, add an S:
    for iOut = 1:length(par.code.outcome)
        par.eventCode{iOut} = sprintf('S%d', par.code.outcome(iOut)); % s in ft
    end

    % ------------------------------------------------------------------- %
    %% Retrieve subject-specific EEG directory:
    ...
    for iBlock = 1:nBlock % iBlock = 1;
        ...
        cfg.trialdef            = [];
        cfg.trialdef.eventvalue = par.eventCode; % determined above based on prep input
        ...
        % trigger that follows previous cue trigger within 700 ms (outcome 
        % presentation is 700 ms) and check if trial sequence from behavioural
        ...
        if  ~(iSub ==8) && (ismember(iBlock, [1 2 4 5]) && nEEGTrials > 110) || (ismember(iBlock, [3 6]) && nEEGTrials > 100)
            ...
            % Check if any two adjacent triggers are less than 700 ms
            % apart (outcome offset is after 700 ms):
            tooShortTriggers    = [0; diff(blockData{iBlock}.sampleinfo(:, 1)) < 700];
