...
cat("Load packages\n")
library(car) # Version ‘3.0.8’ # for Anova()
...
fullData <- EEGfMRIPav_2_Preprocess_Automated() # Apply function

# ====================================================================================================================== #
#### Prepare sub-data sets: ####

## 1) Only Session 1
ses1Data <- subset(fullData, Session_n == 1)

## 2) Only session 2
ses2Data <- subset(fullData, Session_n == 2)

## 3) Select only trials with correct Go responses (actual response):
corGoData <- subset(fullData, stim_go_f == "Go" & (is_go_cor_n == 0 | is_go_cor_n == 1 & ACC_cor_n == 1)) # not incorrect Go

## 4) Select only trials with incorrect Go responses (actual response):
incorGoData <- subset(fullData, stim_go_f == "Go" & (is_go_cor_n == 0 | is_go_cor_n == 1 & ACC_cor_n == 0)) # not correct Go

## 5) Select Go cues (to test differences in accuracy between valence):
goData <- subset(fullData, stim_go_f == "Go") # not incorrect Go
...
mod <- glmer(is_go_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, family = binomial())
saveRDS(mod, paste0(modDir, "mod_pGo_cor_valence_action.rds")) # save
mod <- readRDS(paste0(modDir, "mod_pGo_cor_valence_action.rds")) # read in again
summary(mod)
# Fixed effects:
...
## P-values with LRTs:
mod_LRT <- mixed(mod, fullData, family = binomial(), method = "LRT", type = "III",
...
# ====================================================================================================================== #
#### On subset of people used in EEG-fMRI analyses (N = 29): ####

## Select subsect of data;
subsetData <- subset(fullData, !(PPN_n %in% c(11, 12, 15, 23, 25, 26, 30))) # 11 12 15 23 25 26 30
...
fullData$stim_go_win_f <- factor(paste(fullData$stim_go_o, fullData$stim_win_o, sep = "2"))
contrasts(fullData$stim_go_win_f)

## Mixed-effects logistic regression:
mod <- glmer(is_go_cor_n ~ stim_go_win_f + (stim_go_win_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## P-values with LRTs:
mod_LRT <- mixed(mod, ses1Data, family = binomial(), method = "LRT", type = "III",
                 control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#                      Df   Chisq Chi Df   Pr(>Chisq)    
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## P-values with LRTs:
mod_LRT <- mixed(mod, ses2Data, family = binomial(), method = "LRT", type = "III",
                 control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#                      Df   Chisq Chi Df     Pr(>Chisq)    
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_cor_n ~ stim_win_f + (stim_win_f|PPN_f), data = corGoData, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
summary(mod)
#             Estimate Std. Error z value Pr(>|z|)  
...
## P-values with LRTs:
mod_LRT <- mixed(mod, corGoData, family = binomial(), method = "LRT", type = "III",
                 control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#            Df  Chisq Chi Df Pr(>Chisq)    
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_cor_n ~ stim_win_f + (stim_win_f|PPN_f), data = incorGoData, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
summary(mod)
#             Estimate Std. Error z value           Pr(>|z|)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, incorGoData, family = binomial(), method = "LRT", type = "III",
                 control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#            Df  Chisq Chi Df Pr(>Chisq)    
...
table(fullData$is_go_n == fullData$is_go_cor_n) # identical with corrected Go variable

## Mixed effecs logistic regression:
mod <- glmer(is_go_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, family = binomial())
...
# Mixed-effects logistic regression:
mod <- glmer(is_go_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
### Mixed-effects logistic regression:
mod <- glmer(is_go_n ~ stim_win_f + (stim_win_f||PPN_f), data = corGoData, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## Mixed-effects logistic regression:
mod <- glmer(is_go_n ~ stim_win_f + (stim_win_f|PPN_f), data = incorGoData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, family = binomial())
...
fullData$stim_go_win_f <- factor(paste(fullData$stim_go_f, fullData$stim_win_f, sep = "2"))

## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_go_win_f + (stim_go_win_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_win_f + (stim_win_f|PPN_f), data = corGoData, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_win_f + (stim_win_f|PPN_f), data = goData, family = binomial())
summary(mod)
#             Estimate Std. Error z value             Pr(>|z|)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, goData, family = binomial(), method = "LRT", type = "III",
                 control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#            Df  Chisq Chi Df Pr(>Chisq)
...
table(fullData$ACC_n == fullData$ACC_cor_n)
# FALSE  TRUE 
# 13859  9181 

## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, family = binomial())
Anova(mod, type = "III")
#                        Chisq Df Pr(>Chisq)    
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_win_f + (stim_win_f|PPN_f), data = corGoData, family = binomial(),
             control = glmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = ses1Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = ses2Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_cor_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = corGoData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = fullData, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = ses1Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = ses2Data, family = binomial())
...
## Mixed-effects logistic regression:
mod <- glmer(ACC_n ~ stim_cong_f + (stim_cong_f|PPN_f), data = corGoData, family = binomial())
...
mod <- lmer(RT_nna ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, na.action = na.exclude)
...
## P-values with LRTs:
mod_LRT <- mixed(mod, fullData, method = "LRT", type = "III", 
...
## Mixed-effects linear regression: 
mod <- lmer(RT_nna ~ stim_go_win_f + (stim_go_win_f|PPN_f), data = fullData)
...
## P-values with LRTs:
mod_LRT <- mixed(mod, subsetData, method = "LRT", type = "III", 
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
...
## Mixed-effects linear regression:
mod <- lmer(RT_nna ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, na.action = na.exclude)
Anova(mod, type = "III")
#                          Chisq Df            Pr(>Chisq)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, ses1Data, method = "LRT", type = "III", 
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#                      Df   Chisq Chi Df       Pr(>Chisq)    
...
### Mixed-effects linear regression:
mod <- lmer(RT_nna ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, na.action = na.exclude)
Anova(mod, type = "III")
#                         Chisq Df            Pr(>Chisq)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, ses2Data, method = "LRT", type = "III", 
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#                      Df   Chisq Chi Df   Pr(>Chisq)    
...
## Mixed-effects linear regression:
mod <- lmer(RT_nna ~ stim_win_f + (stim_win_f|PPN_f), data = corGoData, na.action = na.exclude)
...
## P-values with LRTs:
mod_LRT <- mixed(mod, corGoData, method = "LRT", type = "III", 
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#            Df  Chisq Chi Df Pr(>Chisq)    
...
## Mixed-effects linear regression:
mod <- lmer(RT_nna ~ stim_win_f + (stim_win_f|PPN_f), data = incorGoData, na.action = na.exclude)
...
## P-values with LRTs:
mod_LRT <- mixed(mod, incorGoData, method = "LRT", type = "III", 
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#            Df  Chisq Chi Df       Pr(>Chisq)    
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = fullData, na.action = na.exclude)
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses1Data, na.action = na.exclude)
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_go_f*stim_win_f + (stim_go_f*stim_win_f|PPN_f), data = ses2Data, na.action = na.exclude)
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_win_f + (stim_win_f|PPN_f), data = corGoData, na.action = na.exclude)
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_win_f + (stim_win_f|PPN_f), data = incorGoData, na.action = na.exclude)
...
fullData$stim_go_acc <- factor(ifelse(fullData$stim_go_f == "NoGo", "NoGo",
                                    ifelse(fullData$stim_go_f == "Go" & fullData$ACC_cor_n == 1, "Correct Go",
                                           ifelse(fullData$stim_go_f == "Go" & fullData$ACC_cor_n == 0, "Incorrect Go",
                                                  NA))))
sum(is.na(fullData$stim_go_acc)) # No NAs
table(fullData$stim_go_acc)
# correct Go incorrect Go         NoGo 
#       4303         7217        11520 

## Plot:
ggplot(fullData,aes(stim_go_acc, RT_nna)) + 
...
## Mixed-effects linear regression:
mod <- lmer(RT ~ stim_go_acc + (stim_go_acc|PPN_f), data = fullData, na.action = na.exclude)
summary(mod)
...
max(fullData$RT_nna, na.rm = T) # 1.3035 --> maximum otherwise NaN
tapply(fullData$RT, fullData$response_f, mean, na.rm = T)
#      Left      NoGo     Right 
# 0.8280673       NaN 0.8063441
tapply(fullData$RT_nna, fullData$response_f, mean, na.rm = T)
#      Left      NoGo     Right 
# 0.7729851       NaN 0.7447934

## With RT:
mod <- lmer(RT ~ response_f + (response_f|PPN_f), data = fullData)
summary(mod)
#              Estimate Std. Error        df t value            Pr(>|t|)    
...
## With RT_nna:
mod <- lmer(RT_nna ~ response_f + (response_f|PPN_f), data = fullData)
summary(mod)
#              Estimate Std. Error        df t value            Pr(>|t|)    
...
tapply(fullData$RT, fullData$ACC_f, mean, na.rm = T)
#   Correct Incorrect 
# 0.7840502 0.8412819 

mod <- lmer(RT ~ ACC_f + (ACC_f|PPN_f), fullData)
summary(mod)
#              Estimate Std. Error        df t value             Pr(>|t|)    
...
fullData$ACC_types_n <- ifelse(fullData$stim_go_n == 1 & fullData$is_go_cor_n == 1 & fullData$ACC_cor_n == 1, 1,
                             ifelse(fullData$stim_go_n == 1 & fullData$is_go_cor_n == 1 & fullData$ACC_cor_n == 0, 2,
                                    ifelse(fullData$stim_go_n == 0 & fullData$is_go_cor_n == 1 & fullData$ACC_cor_n == 0, 3,
                                           NA)))
table(fullData$ACC_types_n)
fullData$ACC_types_f <- factor(fullData$ACC_types_n, levels = c(1, 2, 3), labels = c("correct", "wrong Go", "Go for NoGo"))
tapply(fullData$RT, fullData$ACC_types_f, mean, na.rm = T)
#   correct    wrong Go Go for NoGo 
# 0.7273191   0.7497505   0.8169105 

## Model:
mod <- lmer(RT ~ ACC_types_f + (ACC_types_f|PPN_f), fullData)
summary(mod)
#               Estimate Std. Error        df t value             Pr(>|t|)    
...
goRespData <- droplevels(subset(fullData, ACC_types_f %in% c("correct", "wrong Go")))
levels(goRespData$ACC_types_f)
mod <- lmer(RT ~ ACC_types_f + (ACC_types_f|PPN_f), goRespData)
summary(mod)
#               Estimate Std. Error        df t value            Pr(>|t|)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, goRespData, method = "LRT", type = "III",
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#             Df  Chisq Chi Df Pr(>Chisq)
...
wrongGoData <- droplevels(subset(fullData, ACC_types_f %in% c("wrong Go", "Go for NoGo")))
levels(wrongGoData$ACC_types_f)
mod <- lmer(RT ~ ACC_types_f + (ACC_types_f|PPN_f), wrongGoData)
summary(mod)
#               Estimate Std. Error        df t value             Pr(>|t|)    
...
## P-values with LRTs:
mod_LRT <- mixed(mod, wrongGoData, method = "LRT", type = "III",
                 control = lmerControl(optCtrl = list(maxfun = 1e+9), optimizer = c("bobyqa")))
anova(mod_LRT)
#             Df  Chisq Chi Df  Pr(>Chisq)    
