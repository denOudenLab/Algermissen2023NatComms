...
dirs.mat      = fullfile(dirs.behav, 'Data_beh_mat'); % where the data comes from
dirs.csv      = fullfile(dirs.behav, 'Data_beh_csv'); % where the data goes to

% ----------------------------------------------------------------------- %
%% Extract subject numbers:

fprintf('Detect files and extract subject numbers\n');
subVec  = dir(fullfile(dirs.mat, '*_results*')); % select proper files
subVec  = {subVec.name}; % keep only column "name"
nSub    = length(subVec);
subList = nan(nSub, 1);

fprintf('Found %d subjects\n', nSub);

for iSub = 1:nSub % 1 till number of elements in subVec
    subList(iSub) = str2double(string(extractBetween(subVec{iSub}, 'dij_', '_001_results')));  % extracts the actual "sub" number of each subject --> check which ones found
end
...
for iSub = 1:nSub % for each subject
    ...
    load(fullfile(dirs.mat, subVec{iSub})); % load input data
    ...
    for iSes = 1:2 % for each session
        ...
        csvwrite(fullfile(dirs.csv, targetFile), data); % write file as .csv
